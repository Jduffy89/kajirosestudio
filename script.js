const products = [
  {id:'blanket', name:'Custom Blanket', category:'blankets', icon:'🧺', desc:'Our #1 seller — personalize it with photos, names, artwork, and more.', badge:'#1 Seller', variants:[['Baby — 30×40 in.',25],['Toddler / Throw — 40×50 in.',35],['Twin / Throw — 50×60 in.',45],['Full / Queen — 60×80 in.',55]], image:'assets/product-blanket-logo.jpg'},
  {id:'onesie', name:'Baby Onesie', category:'clothing', icon:'👶', desc:'Personalized baby onesie for everyday wear, keepsakes, and gifts.', variants:[['0–3 Month',15],['3–6 Month',15],['6–9 Month',15],['9–12 Month',15],['1–2Y',15]], image:'assets/product-baby-onesie.jpg'},
  {id:'kids-shirt', name:'Kids T-Shirt', category:'clothing', icon:'👕', desc:'Custom kids shirt with your photos, names, artwork, or theme.', variants:[['2Y',18],['3Y',18],['4Y',18],['5/6Y',18],['7/8Y',18],['10/12Y',18]], image:'assets/product-kids-tshirt.jpg'},
  {id:'adult-shirt', name:'Adult T-Shirt', category:'clothing', icon:'👚', desc:'Custom adult shirt designed around your request.', variants:[['Small',25],['Medium',25],['Large',25],['XL',25],['XXL',25],['XXXL',25]], image:'assets/product-adult-tshirt.jpg'},
  {id:'tumbler', name:'Stainless Steel Tumbler', category:'drinkware', icon:'🥤', desc:'Custom tumbler with optional accessory bundle.', variants:[['20 oz',20],['20 oz + accessories',25],['30 oz',30],['30 oz + accessories',35],['40 oz',40],['40 oz + accessories',45]], image:'assets/product-tumbler-logo.jpg'},
  {id:'water-bottle', name:'Plastic Water Bottle', category:'drinkware', icon:'💧', desc:'Custom 20–25 oz plastic water bottle personalized with your design.', variants:[['20–25 oz',15]], image:'assets/product-water-bottle-plastic.jpg'},
  {id:'beach-towel', name:'Beach Towel', category:'accessories', icon:'🏖️', desc:'Personalized beach towel for trips, gifts, and family fun.', variants:[['Standard',20]], image:'assets/product-beach-towel-logo.jpg'},
  ...
  {id:'tote', name:'Tote Bag', category:'bags', icon:'👜', desc:'13×15 in. custom tote bag.', variants:[['13×15 in.',20]], image:'assets/product-tote-logo.jpg'},
  {id:'accessories', name:'Accessories', category:'accessories', icon:'🎁', desc:'Personalized keepsakes and everyday accessories. Choose the item you would like us to customize.', variants:[['Keychain',10],['Pocket Notebook — 5.5×3.5 in.',null],['Medium Spiral Notebook — 5×8 in.',null]], image:'assets/category-accessories-combined.jpg'},
  {id:'other', name:'Other Custom Item', category:'other', icon:'✨', desc:"Looking for something unique? We customize almost anything—just ask! Let's build it together.", variants:[['Custom Request',null]], image:'assets/kaji-rose-logo.png'},
];
...
function updateVariants(){const p=products.find(p=>p.id===productSelect.value)||products[0];variantSelect.innerHTML=p.variants.map((v,i)=>`<option value="${i}">${v[0]} — ${money(v[1])}</option>`).join('');updatePrice();}
...
requestForm.addEventListener('submit',async e=>{
  ...
  const summary=`KAJI ROSE STUDIO\nCUSTOM DESIGN REQUEST ${requestNo}\n\nCustomer: ${document.getElementById('customerName').value}\nEmail: ${document.getElementById('customerEmail').value}\nPhone: ${document.getElementById('customerPhone').value||'Not provided'}\n\nProduct: ${p.name}\nOption / Size: ${v[0]}\nQuantity: ${document.getElementById('quantity').value}\nEstimated item price: ${estimatedPrice.value}\nIntended recipient: ${document.getElementById('intendedFor').value}\nOccasion / purpose: ${document.getElementById('occasionPurpose').value}\nPreferred color scheme: ${document.getElementById('colorScheme').value||'No preference provided'}\nDeadline: ${document.getElementById('designDeadline').value||'No deadline provided'}\nFavorite things / interests: ${document.getElementById('favoriteThings').value||'Not provided'}\nThings to avoid: ${document.getElementById('designAvoid').value||'None provided'}\n\nCustom text: ${customText.value||'None'}\nAdditional design notes: ${document.getElementById('designNotes').value||'None'}\n\nI will attach any photos/artwork directly to this Messenger conversation.\n\nRights confirmation: Yes\nCustom-order process acknowledged: Yes`;
  try{await navigator.clipboard.writeText(summary);}catch(_){/* Clipboard may be blocked; summary is still shown below. */}
  statusBox.className='form-status success';
  statusBox.innerHTML=`<strong>Request ${requestNo} is ready.</strong><br>Your order summary was prepared${navigator.clipboard?' and copied to your clipboard':''}. Open Messenger, paste the summary, then attach any photos/artwork you want Kaji Rose Studio to use.<textarea class="summary-box" readonly>${summary}</textarea>`;
